...
const HELP_MSG = `Hi I am a WhatsApp Bot
...
*Translation:*
_Trans._ como estas
_Trans._ to spanish how are you
...
client.on("message", (msg) => {
  ...
  if (query.substring(0, 7) == "google.") {
    ...
    query.substring(0, 7) == "search." ||
    msg.body.replace(0, 11) == "duckduckgo."
  ) {
...
function translateText(query, id) {
  ...
  if (fromIdx != -1) {
    words.splice(fromIdx, 2);
  }
  if (toIdx != -1) {
    words.splice(toIdx, 2);
  }
  query = words.join(" ");
  translate(query, { from: from, to: to })
    .then((res) => {
...
function calculate(query, id) {
  ...
  try {
    ...
    client.sendMessage(
      id,
      "Cannot recognise your Problem. Searching on Google...",
    );
    searchGoogle(qu, id);
  }
}
  ...
  request(
    ...
    async (err, response, body) => {
      ...
      try {
        let ytLink = `http://www.youtube.com/watch?v=${videoID}`;
        let res = await ytdl.getBasicInfo(ytLink);
        let fileName = res.videoDetails.title
          .substring(0, 70)
          .replace(/\s/g, "_");
        ...
        video.on("end", () => {
          let audMedia = MessageMedia.fromFilePath(
            filepath.replace(/\\/g, "/"),
          );
